...
export async function createInvoice({ invoice_no, customer_code, sales_person_code, invoice_date, vat_rate, line_items }) {
  ...
  try {
    ...
    const resolvedSalesCode = String(sales_person_code || "").trim();
    if (resolvedSalesCode) {
      const sp = await client.query("SELECT id FROM sales_person WHERE code = $1", [resolvedSalesCode]);
      if (sp.rowCount === 0) throw new Error(`Sales person not found: ${resolvedSalesCode}`);
